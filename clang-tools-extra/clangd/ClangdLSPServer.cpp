...
void ClangdLSPServer::onDocumentSymbol(const DocumentSymbolParams &Params,
  ...
  Server->documentSymbols(
      ...
      [this, FileURI, Reply = std::move(Reply)](
        ...
        for(DocumentSymbol &docsymb:*Items)
        {
          if(!isValidUtf8(docsymb.detail))
          docsymb.detail = boost::locale::conv::to_utf<char>(docsymb.detail,"GB18030");
        }
  ...
  Server->signatureHelp(Params.textDocument.uri.file(), Params.position,
                        [Reply = std::move(Reply), this](
                          ...
                          for (auto &SigInfo : Signature->signatures) {
                            if(!isValidUtf8(SigInfo.documentation))
                            SigInfo.documentation = boost::locale::conv::to_utf<char>(SigInfo.documentation,"gb18030");
                          }
...
void ClangdLSPServer::onHover(const TextDocumentPositionParams &Params,
  ...
  Server->findHover(Params.textDocument.uri.file(), Params.position,
                    [Reply = std::move(Reply), this](
                      ...
                      if(!isValidUtf8((*H)->Documentation))
                      (*H)->Documentation = boost::locale::conv::to_utf<char>((*H)->Documentation,"gb18030");
